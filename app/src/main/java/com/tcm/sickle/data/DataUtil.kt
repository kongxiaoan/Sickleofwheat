...
import com.tcm.sickle.SFApplication
...
object DataUtil {
    ...
    fun getTiktokDataFromAssets(): List<TiktokBean> {
        try {
            if (tiktokData == null) {
                val `is` = SFApplication.context.assets.open("tiktok_data")
                val length = `is`.available()
                val buffer = ByteArray(length)
                `is`.read(buffer)
                `is`.close()
                val result = String(buffer, Charset.forName("UTF-8"))
                Log.d("DataParse", result)
                tiktokData = arrayTiktokBeanFromData(result)
            }
            return tiktokData ?: emptyList()
        } catch (e: IOException) {
            e.printStackTrace()
        }
        return emptyList()
    ...
    fun arrayTiktokBeanFromData(str: String?): List<TiktokBean> {
        val listType: Type = object : TypeToken<ArrayList<TiktokBean?>?>() {}.type
